...
// Answer: it returns the Optional
 ...
 */
export const getOrElse4 = <A>(oa: Optional<A>, other: A): A =>
  oa.fold(
    Fun.constant(other),
    Fun.identity
  );
...
export const replaceWith9 = (numbers: number[]): number[] =>
  Arr.map(numbers, Fun.constant(9));

// TODO: In the previous question, what's the *same* between the input and output values
// Answer: Returning the same number of element and the type
...
export const doubleTwice: (x: number) => number = Fun.compose1(dbl, dbl);
